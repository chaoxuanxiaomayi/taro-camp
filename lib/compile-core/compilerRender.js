...
function generateJSXAttr(ast) {
  const code = (0, _generator.default)(ast).code;
  return code.replace(/(this\.props\.)|(this\.state\.)/g, "").replace(/(props\.)|(state\.)/g, "").replace(/this\./g, "");
}
...
function parseJSXElement(element) {
  ...
  const {
    attributes,
    name
  } = element.openingElement; // 标签名

  const componentName = name.name; // 属性键值对
    ...
    attributesTrans = attributes.reduce((obj, attr) => {
      ...
      if (typeof name === "string") {
        if (t.isStringLiteral(attrValue)) {
          ...
          let code = (0, _generator.default)(attrValue.expression, {
            quotes: "single",
            concise: true
          }).code.replace(/"/g, "'").replace(/(this\.props\.)|(this\.state\.)/g, "").replace(/this\./g, ""); // 如果是事件则函数名，否则是绑定值
...
function setCustomEvent(renderPath) {
  const classPath = renderPath.findParent(p => p.isClassExpression() || p.isClassDeclaration()); // 给类组件增加event属性

  const eventPropName = "$$events";

  const _usedEvents = Array.from(usedEvents).map(s => t.stringLiteral(s)); // 变成数组array插入


  let classProp = t.classProperty(t.identifier(eventPropName), t.arrayExpression(_usedEvents));
  classProp.static = true; // 插入class属性

  classPath.node.body.body.unshift(classProp);
}

function compileRender(renderPath) {
  //  防止无限递归用
  ...
  renderPath.traverse({
    JSXAttribute(path) {
      ...
      if (attributeName === "onClick" && t.isJSXExpressionContainer(value)) {
        ...
        const methodName = (0, _utils.findMethodName)(path.node.value.expression);
  ...
  renderPath.traverse({
    JSXElement(path) {
      if (t.isReturnStatement(path.parent) && !finalReturnElement) {
        ...
        const block = (0, _utils.buildBlockElement)([t.jSXAttribute(t.jSXIdentifier("wx:if"), t.jSXExpressionContainer(t.identifier("$taroCompReady")))]);
        finalReturnElement = block; // 把原先的节点变为其子节点

        block.children.push(path.node); //  替换模板中this.state this.props
  ...
  renderPath.traverse({
    BlockStatement(path) {
      ...
      path.node.body.unshift((0, _template.default)(`
        this.__state = arguments[0];
      `)()); // 让最后一句合并state

      path.node.body[path.node.body.length - 1] = (0, _template.default)(`
        Object.assign(this.__state, {
          ${vars.map(i => `${i}: ${i},`).join(",")}
        });
      `)(); //最后插入返回

      path.node.body.push((0, _template.default)(`
        return this.__state
      `)());
    }
